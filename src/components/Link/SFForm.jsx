...
const LinkForm = ({ usData, data, localities, closeModal }) => {
	const fD = usData;
	const { createLink, updateLink } = useAuth();
	const { values, handleChange, handleBlur, handleSubmit, isSubmitting, setSubmitting, errors, touched, setFieldValue } = useFormik({
		...
		onSubmit: async (formData) => {
			setSubmitting(true);
			if (fD?.id){
				const { isError, data, error } = await updateLink(fD.id, formData);
				setSubmitting(false);
				...
				const { isError, data, error } = await createLink(formData);
				setSubmitting(false);
				if (isError) toast.error(error.message);
				else {
					toast.success("success");
