...
import { Loader, MousePointer, Rss } from "react-feather";
...
import SFForm from "components/ScannerFeed/SFForm";
import StyledInput, { StyledSelect } from "components/form/StyledInput";

const ScannerFeedPage = () => {
    const { fetchSFs, fetchAdminLocality, currentUser } = useAuth();
    ...
    const FetchData = async (l=null) => {
        const { isError, data, error } = await fetchSFs(l, page, limit);
        if (isError) toast.error(error.message);
        setDatas(data.data)
        setPages(data.pages)
        setLoading(false);
    ...
    const routePage = (id, m) => {
        if (id) return router.push(`/${Pages.SCANNER_FEED}/${id}`);
    }
    const pageNavigate = (page) => setPage(page);

    return (
        <DashboardLayout pageTitle={"Scanner Feeds"}>
            <div className="hk-pg-header">
                <h4 className="hk-pg-title">
                    <span className="pg-title-icon"><Rss /></span>
                    Scanner Feeds
                </h4>
                <div className="d-flex">
					<button onClick={()=>SetModalView(true, <SFForm data={null} localities={localities} closeModal={CloseModal}/>)} className="btn btn-primary btn-sm">Create Scanner Feed</button>
                </div>
            </div>
			...
			{showEdit && <CustomModal centered={true} component={component} show={showEdit} title={"Create Scanner Feed"} closeModal={()=>CloseModal()}/>}
        </DashboardLayout>
    )
};

export default withPrivateRoute(ScannerFeedPage);
