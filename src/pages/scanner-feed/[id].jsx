...
const ScannerFeedDetails = () => {
	...
	const { fetchOneSF, deleteSF, fetchAdminLocality, currentUser } = useAuth();
	...
	const FetchData = async () => {
		// if (id){
			const { isError, data, error } = await fetchOneSF(id);
			if (isError) {
	...
	const Remove = async () => {
        setConfirming(true);
        const { isError, data, error } = await deleteSF(id);
        if (isError) toast.error(error.message);
	...
	return(
		<DashboardLayout pageTitle={"Scanner Feeds"} isLoading={isLoading}>
			<div className="hk-pg-header mb-10">
				...
				<div className="d-flex button-list">
					<button onClick={()=>SetModalView(true, <SFForm usData={{id}} data={userData} localities={localities} closeModal={()=>SetModalView(false, null)}/>)} className="btn btn-info btn-sm">Edit</button>
			...
			{showEdit && <CustomModal centered={true} component={component} show={showEdit} title={"Edit Scanner Feed"} closeModal={()=>SetModalView(false, null)}/>}
		</DashboardLayout>
	)
};
