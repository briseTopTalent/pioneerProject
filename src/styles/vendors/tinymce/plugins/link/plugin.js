	...
	var link = (function () {
		...
		var link = function (editor, attachState) {
			return function (data) {
				editor.undoManager.transact(function () {
					...
					if (
						!$_4b6xsbfujm0ofzch.hasRelList(editor.settings) &&
						$_4b6xsbfujm0ofzch.allowUnsafeLinkTarget(editor.settings) === false
					) {
		...
		var showDialog = function (editor, linkList) {
			...
			if ($_4b6xsbfujm0ofzch.hasRelList(editor.settings)) {
				relListCtrl = {
					...
					label: "Rel",
					values: buildListItems(
						$_4b6xsbfujm0ofzch.getRelList(editor.settings),
						function (item) {
							if (
								$_4b6xsbfujm0ofzch.allowUnsafeLinkTarget(editor.settings) === false
							) {
								item.value = $_d522tkfzjm0ofzda.toggleTargetRules(
									item.value,
									data.target === "_blank"
								);
							}
			...
			if ($_4b6xsbfujm0ofzch.hasLinkClassList(editor.settings)) {
				classListCtrl = {
					...
					label: "Class",
					values: buildListItems(
						$_4b6xsbfujm0ofzch.getLinkClassList(editor.settings),
						function (item) {
							if (item.value) {
			...
			win = editor.windowManager.open({
				...
				onSubmit: function (e) {
					var assumeExternalTargets = $_4b6xsbfujm0ofzch.assumeExternalTargets(
						editor.settings
					);
		...
		var leftClickedOnAHref = function (editor) {
			return function (elm) {
				var sel, rng, node;
				if (
					$_4b6xsbfujm0ofzch.hasContextToolbar(editor.settings) &&
					!isContextMenuVisible(editor) &&
					$_d522tkfzjm0ofzda.isLink(elm)
				) {
		...
		var toggleActiveState = function (editor) {
			return function () {
				...
				editor.on("nodechange", function (e) {
					self.active(
						!editor.readonly && !!$_d522tkfzjm0ofzda.getAnchorElement(editor, e.element)
					);
				});
			};
		};
		var toggleViewLinkState = function (editor) {
			return function () {
				...
				if (
					!$_d522tkfzjm0ofzda.hasLinks(editor.dom.getParents(editor.selection.getStart()))
				) {
					self.hide();
				}
		...
		var setupContextToolbars = function (editor) {
			if (editor.addContextToolbar) {
				editor.addContextToolbar(
					$_c3czoyfsjm0ofzcd.leftClickedOnAHref(editor),
