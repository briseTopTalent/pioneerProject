	...
	var image = (function () {
			...
			return function (evt) {
				...
				if (
					...
					if (css["margin-top"] === css["margin-bottom"]) {
						rootControl
							.find("#vspace")
							.value($_5pkwh7ctjm0ofykd.removePixelSuffix(css["margin-top"]));
					} else {
						rootControl.find("#vspace").value("");
					}
					if (css["margin-right"] === css["margin-left"]) {
						rootControl
							.find("#hspace")
							.value($_5pkwh7ctjm0ofykd.removePixelSuffix(css["margin-right"]));
					} else {
						rootControl.find("#hspace").value("");
					}
				}
				if (css["border-width"]) {
					rootControl
						.find("#border")
						.value($_5pkwh7ctjm0ofykd.removePixelSuffix(css["border-width"]));
				} else {
		...
		var onSrcChange = function (evt, editor) {
			...
			if (!meta.width && !meta.height) {
				...
				$_5pkwh7ctjm0ofykd.getImageSize(
		...
		var getGeneralItems = function (editor, imageListCtrl) {
			...
			if ($_95hfkhcsjm0ofyka.getClassList(editor)) {
				generalFormItems.push({
					...
					type: "listbox",
					label: "Class",
					values: $_5pkwh7ctjm0ofykd.buildListItems(
						$_95hfkhcsjm0ofyka.getClassList(editor),
						function (item) {
							if (item.value) {
		...
		function Dialog(editor) {
			function showDialog(imageList) {
				...
				if (
					$_95hfkhcsjm0ofyka.hasAdvTab(editor) ||
					$_95hfkhcsjm0ofyka.hasUploadUrl(editor) ||
					$_95hfkhcsjm0ofyka.hasUploadHandler(editor)
				) {
