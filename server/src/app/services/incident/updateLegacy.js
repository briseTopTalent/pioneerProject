...
if (
  ...
  shout(
    'NEVER_RUN_LEGACY_HOOK DETECTED! NOT RUNNING LEGACY INCIDENT ***UPDATE***'
  );
  module.exports = () => {
    shout('not actually sending legacy axios post (UPDATE)');
  };
  return;
} else {
  const UpdateLegacyIncident = async incident => {
    try {
      ...
      axios
        .post(process.env.legacy + '/update', legacy)
  ...
  module.exports = UpdateLegacyIncident;
}
